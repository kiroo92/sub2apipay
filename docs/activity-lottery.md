...
> 状态：已实施
> 目标：按月卡购买与 Codex 消费持续发放摇摇卡，并通过用户级大奖限制与全局库存控制成本
> 数据原则：用户、订单、消费和订阅信息以 Sub2API 为唯一事实来源；本地只保存摇奖、发奖和人工兑换所必需的审计记录

## 1. 活动规则

活动期内按三个来源持续获得摇摇卡，不设置用户总张数上限：

| 来源                  | 发卡规则                        |
| --------------------- | ------------------------------- |
| 轻享 / 尊享月卡       | 每笔已完成且未退款的购买赠 1 张 |
| 非月卡套餐 Codex 消费 | 每累计 `$2400` 赠 1 张          |
| 余额 Codex 消费       | 每累计 `$2000` 赠 1 张          |

月卡订单必须属于当前用户、状态为 `COMPLETED`、`refund_amount = 0`，且 `paid_at` 位于活动窗口。套餐和余额消费分别读取 Sub2API 用量统计；月卡分组的消费从套餐消费中扣除。

摇摇卡资格不落本地票券表。页面加载及每次摇奖前，服务端重新计算：

```text
earned = monthCardOrders + floor(packageUsage / 2400) + floor(balanceUsage / 2000)
...
奖池是 10,000 份有限库存。每次从当前剩余库存中安全随机抽取，实际概率等于该奖项剩余库存占当前用户可用库存的比例。

| 奖励         | 初始库存 | 发放方式                  |
| ------------ | -------: | ------------------------- |
| `$30` 额度   |     4800 | 自动增加 Sub2API 账户余额 |
| `$60` 额度   |     3000 | 自动增加 Sub2API 账户余额 |
| `$120` 额度  |     1500 | 自动增加 Sub2API 账户余额 |
| `$240` 额度  |      100 | 自动增加 Sub2API 账户余额 |
| 再摇一次     |      590 | 不扣除本次摇摇卡          |
| 免费重置额度 |       10 | 用户联系管理员人工兑换    |

### 2.1 大奖与重置资格

`$240` 额度和免费重置额度属于大奖组。同一用户一旦获得任一大奖，其后续奖池会同时移除全部大奖，大奖概率降为 0。免费重置额度只对当前存在有效订阅的用户开放。

每个奖项的历史发放数达到初始库存后立即从全局奖池移除。库存核对和记录写入位于同一 PostgreSQL 事务与全局 advisory lock 内，多实例并发时也不会超发。
...
| 月卡订单 | `GET /api/v1/admin/payment/orders`，按 `user_id` 查询并分页     |
| 套餐定义 | `GET /api/v1/admin/payment/plans`                               |
| 消费统计 | `GET /api/v1/admin/usage/stats`，按用户、计费类型和活动时间查询 |
| 有效订阅 | `GET /api/v1/admin/users/{id}/subscriptions`                    |
| 额度发放 | `POST /api/v1/admin/users/{id}/balance`                         |

订单资格严格检查状态、退款金额、订单类型和付款时间。用量统计请求使用 `nocache=true`，摇奖前读取最新消费数据。
...
2. 读取月卡订单、套餐定义、两类消费统计和有效订阅，重新计算摇摇卡。
3. 开启数据库事务，先获取全局奖池 lock，再获取当前用户 lock。
4. 相同 `requestId` 已存在时直接返回原结果。
5. 再读本地记录，校验 `used < earned`。
6. 统计全局各奖项已发数量，移除库存耗尽奖项。
7. 根据有效订阅与个人大奖历史移除当前用户不具备资格的奖项。
8. 使用服务端安全随机数按剩余库存权重确定奖项并写入记录。
9. 额度奖调用 Sub2API `addBalance`，重置奖励进入 `MANUAL_PENDING`，“再摇一次”直接标记 `ISSUED`。
...
`GET /api/lottery` 返回三类发卡进度、累计卡片、剩余卡片、奖池余量、摇奖记录及是否存在有效订阅。接口不返回订单列表、订阅详情、随机种子或内部错误。
...
用户页面第一屏直接呈现转盘。左侧为摇奖操作，右侧展示三类发卡进度、可用摇摇卡和全局奖池余量，底部为完整摇奖记录。
